...
function chooseExec() {
  ...
  switch (process.platform) {
    case 'win32':
      tidyExe = path.join('../../../bin','win32','tidy.exe');
      break;
    case 'linux':
        if (process.env.HOME === '/Users/jmhmd') {
        // running on kodingen most likely
            tidyExe = path.join('../../../vendor','tidy_koding','bin','tidy');
        } else {
        // running on heroku most likely
            tidyExe = path.join('../../../vendor','tidy','bin','tidy');
        }
      break;
  	case 'darwin':
	    tidyExe = path.join('/usr/bin', 'tidy');
	    break;
    default:
